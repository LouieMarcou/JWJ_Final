...
-(void)showMusicLibrary:(bool)shouldAppendToPlaylist
{
    ...
    self.myPicker = [[MPMediaPickerController alloc] initWithMediaTypes:MPMediaTypeMusic];
    self.myPicker.delegate = self;
    self.myPicker.allowsPickingMultipleItems = NO;
    self.myPicker.showsCloudItems = NO;
    self.myPicker.showsItemsWithProtectedAssets = NO;
    ...
    [containerController.view addSubview:self.myPicker.view];
    
    [UnityGetGLViewController() presentViewController:self.myPicker animated:YES completion:nil];
}
...
{
    NSLog(@"reached didpickmediaitems");
...
-(void)getPlaylists
{
    // use native player for playlists
    NSUserDefaults *myDefaults = [NSUserDefaults standardUserDefaults];
    [myDefaults setObject:@"0" forKey:@"isLoadingAudioClip"];
    [myDefaults synchronize];
    
    MPMediaQuery *playlistsQuery = [MPMediaQuery playlistsQuery];
    [playlistsQuery setGroupingType:MPMediaGroupingPlaylist];
    NSArray *mediaCollections = [playlistsQuery collections];
    self.allItemsDictionary = [[NSMutableDictionary alloc] init];
    
    for(int i=0; i<mediaCollections.count; i++)
    {
        MPMediaItemCollection *playlistCollection = mediaCollections[i];
        
        NSMutableArray *playlistItemsArray = [[NSMutableArray alloc] init];
        
        for(int j=0; j<playlistCollection.count; j++)
        {
            [playlistItemsArray addObject:playlistCollection.items[j]];
        }
        
        //[self.allItemsDictionary setObject:playlistItemsArray forKey:[NSString stringWithFormat:@"%llu", playlistCollection.persistentID]];
        
        [self.allItemsDictionary setObject:playlistItemsArray forKey:[NSString stringWithFormat:@"%@", [playlistCollection valueForProperty:MPMediaPlaylistPropertyName]]];
        
        UnitySendMessage("iOSMusic", "ExtractPlaylists", [[NSString stringWithFormat:@"%@", [playlistCollection valueForProperty:MPMediaPlaylistPropertyName]] UTF8String]);
    }
}
...
-(void)getLibraryContents
{
    ...
    [allSongsQuery addFilterPredicate:[MPMediaPropertyPredicate predicateWithValue:[NSNumber numberWithBool:NO] forProperty:MPMediaItemPropertyIsCloudItem]];

    [allSongsQuery setGroupingType:MPMediaGroupingArtist];
    NSArray *mediaCollections = [allSongsQuery collections];
    self.allItemsDictionary = [[NSMutableDictionary alloc] init];
    
    for(int i=0; i<mediaCollections.count; i++)
    {
        MPMediaItemCollection *songCollection = mediaCollections[i];
        
        for(int j=0; j<songCollection.count; j++)
        {
            UnitySendMessage("iOSMusic", "ExtractLibraryMediaItem", [[NSString stringWithFormat:@"%@", [songCollection.items[j] valueForProperty:MPMediaItemPropertyPersistentID]] UTF8String]);
            
            [self extractMetaData:songCollection.items[j]];
            
            UnitySendMessage("iOSMusic", "FinalizeItemExtraction", [[NSString stringWithFormat:@"%@", [songCollection.items[j] valueForProperty:MPMediaItemPropertyPersistentID]] UTF8String]);
        }
    }
    
    UnitySendMessage("iOSMusic", "HandleEndOfLibraryExtraction", "");
}
...
-(void)playItemWithPersistentID:(NSString*)itemPersistentID;
{
    ...
    predicate = [MPMediaPropertyPredicate predicateWithValue:itemPersistentID forProperty:MPMediaItemPropertyPersistentID];
    songQuery = [[MPMediaQuery alloc] init];
    [songQuery addFilterPredicate:predicate];
    if (songQuery.items.count > 0)
    {
        // found the song by its persistent ID
        NSUserDefaults *myDefaults = [NSUserDefaults standardUserDefaults];
        [myDefaults setObject:@"0" forKey:@"isLoadingAudioClip"];
        [myDefaults synchronize];
        
        NSError *err;
        self.myPlayer = [[AVAudioPlayer alloc] initWithContentsOfURL:[[songQuery.items objectAtIndex:0] assetURL] error:&err];
        
        if(err)
        {
            NSLog(@"Error when attempting to play a playlist item %@ %@: %@", [[songQuery.items objectAtIndex:0] artist], [[songQuery.items objectAtIndex:0] title], err);
        }
...
{
    // persistent ID fallback approach
    //NSNumberFormatter * numberFormatter = [[NSNumberFormatter alloc] init];
    //NSNumber *incomingPersistentID = [numberFormatter numberFromString:persistentID];
    //NSLog(@"%llu", [incomingPersistentID unsignedLongLongValue]);
    ...
    for(int i=0; i<self.allItemsDictionary.allKeys.count; i++)
    {
        NSString *userPlaylistName = self.allItemsDictionary.allKeys[i];
        
        if([userPlaylistName isEqual:playlistName])
        {
            // found the queried playlist, now start playback of its items
            self.myPlaylist = [[NSMutableArray alloc] initWithArray:[self.allItemsDictionary objectForKey:self.allItemsDictionary.allKeys[i]]];
            
            NSError *err;
            self.myPlayer = [[AVAudioPlayer alloc] initWithContentsOfURL:[[self.myPlaylist objectAtIndex:0] assetURL] error:&err];
            
            if(err)
            {
                NSLog(@"Error when attempting to play a playlist item %@ %@: %@", [[self.myPlaylist objectAtIndex:0] artist], [[self.myPlaylist objectAtIndex:0] title], err);
            }
            ...
            [self extractMetaData:[self.myPlaylist objectAtIndex:0]];
        }
    }
}
...
-(void)randomSongWithNativeAudioPlayer
{
    ...
    NSArray *mediaCollections = [songsQuery collections];
    ...
    for(int i=0; i<mediaCollections.count; i++)
    {
        ...
        for(int j=0; j<songCollection.items.count; j++)
        {
            [allItems addObject:songCollection.items[j]];
        }
    }
    
    MPMediaItem *chosenItem = (MPMediaItem*)[allItems objectAtIndex:(arc4random() % [allItems count])];
    
    NSError *err;
    self.myPlayer = [[AVAudioPlayer alloc] initWithContentsOfURL:[chosenItem assetURL] error:&err];
    
    if(err)
    {
        NSLog(@"Error when randomly choosing %@ %@: %@", [chosenItem artist], [chosenItem title], err);
    }
    ...
    [self extractMetaData:chosenItem];
...
{
    MPMediaQuery *songsQuery = [MPMediaQuery songsQuery];
    [songsQuery addFilterPredicate:[MPMediaPropertyPredicate predicateWithValue:@(NO)
    ...
    [self exportSong:[allItems objectAtIndex:(arc4random() % [allItems count])]];
}
